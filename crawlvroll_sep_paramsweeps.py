...
def plotbothsidesheatmap(n_t,rEms,pulse_vals,contra_weights,offsetcontra,contra_dur,inhib_pulse,inhib_side,perturb_input,plotti,ti):
    f,(ax1,ax2) = plt.subplots(nrows=2)
    sb.heatmap(rEms[n_t,:,0],ax=ax1,cbar=False)
    ax1.set(ylabel="Segments", title = plotti)
    ax1.set_xticks([])
    sb.heatmap(rEms[n_t,:,1],ax=ax2,cbar=False)
...
def heatmap_params_LRphis(meanphi_LR,varphi_LR,intersegprops,contraprops,stype,plottitle):
    ...
    if stype == 0:
        ...
        tist = "Roll - "
    ax1.set(xlabel="Contralateral Coupling", ylabel="Intersegmental Coupling", title = tist + r"$<\phi>$")
    ax2.set(xlabel="Contralateral Coupling", ylabel="",  title = r"$\sigma^2_\phi$ - ")
...
def heatmap_params_intersegphis(meanphi_interseg,varphi_interseg,intersegprops,contraprops,stype,plottitle):
    f,((ax1,ax2),(ax3,ax4)) = plt.subplots(nrows=2,ncols=2)
    ilabels = [str(round(i,2)) for i in intersegprops]
    clabels = [str(round(c,2)) for c in contraprops]
    sb.heatmap(meanphi_interseg[:,:,0],ax=ax1)
    sb.heatmap(varphi_interseg[:,:,0],ax=ax2)
    sb.heatmap(meanphi_interseg[:,:,1],ax=ax3)
    sb.heatmap(varphi_interseg[:,:,1],ax=ax4)
    if len(contraprops)>10:
        ax1.set_xticks(np.arange(0,len(contraprops),2))
        ax2.set_xticks(np.arange(0,len(contraprops),2))
    ...
    if stype == 0:
        ...
        tist = "Roll - "
    ax1.set(xlabel="", ylabel="Intersegmental Coupling", title = tist + r"$<\phi>$")
    ax2.set(xlabel="", ylabel="",  title = r"$\sigma^2_\phi$ - ")
    ax3.set(xlabel="Contralateral Coupling", ylabel="Intersegmental Coupling")
    ax4.set(xlabel="Contralateral Coupling", ylabel="")
...
def motor_output_check(n_t,E,pulse_vals,c_thresh,titype):
    ...
    if np.where(E[:,:,:] > c_thresh)[0].size>0:
        left_starts = np.ones([E.shape[0],100])*99999
        left_ends = np.ones([E.shape[0],100])*99999
        ...
        if side.size>1:
            right_starts = np.ones([E.shape[0],1000])*99999
            right_ends = np.ones([E.shape[0],1000])*99999
        ...
        phasediff_LR = np.ones([cstart.shape[1],cstart.shape[0]])*99999
        mean_phasediff_LR = np.ones(cstart.shape[1]-1)*99999
        phasediff_interseg = np.ones([cstart.shape[1],cstart.shape[0]-1])*99999
        mean_phasediff_interseg = np.ones(cstart.shape[1]-1)*99999
        if side.size>1:
            for seg in segx:
                #LR comparison
                for wa in np.arange(cstart.shape[1])-2:
                    ...
                    phasediff_LR[wa,seg] = abs(compwavenext - adjval)/abs(adjvalnext - adjval)
                    mean_phasediff_LR[wa] = np.mean(phasediff_LR[wa,:],0) #mean, remove the weird entries
                #interseg comparison
                if seg < E.shape[0]:
                    for si in side:
                        for wa in np.arange(cstart.shape[1])-2:
                            ...
                            comparray_seg = cstart[seg+1,:,si] #compare to all possible waves in this seg on the contralateral side to find nearest
                            adjval = find_nearest(comparray_seg, cstart[seg,wa,si])
                            adjvalnext = find_nearest(comparray_seg, compwavenext)
                            phasediff_interseg[wa,seg] = abs(compwavenext - adjval)/abs(adjvalnext - adjval)
                            mean_phasediff_interseg[wa] = np.mean(phasediff_interseg[wa,:],0) #mean, remove the weird entries
        ...
        if pulse_vals[0,2] == 0: #tonic input, single pulse
            ...
            if n_sides>1:
                I_ext_E[:,:,1] = np.repeat(np.reshape(np.concatenate((np.zeros(rest_dur), round(pulse_vals[0,0] * offsetcontra_sub,2) * np.ones(int(pulse_vals[0,1]*contra_dur_sub)),
                                                                      ...
                                                                      np.zeros(Lt-int((pulse_vals[0,1]*(contra_dur+contra_dur_sub)))))),[Lt,1]),n_segs,axis=1) #contra
...
#%% run simulations - iterate over the parameter space
n_t = np.arange(0,200)
simname = ['crawl', 'roll']
pars = default_pars()
n_sides = 2
...
intersegwprop = np.arange(0.25,5.25,0.25)
pars['wEEadj'] = 20 # keep interseg E the same, but vary I
...
for i in intersegwprop:
    pars['wEIadj'] = -intersegwprop * pars['wEEadj']
    for c,EE in enumerate(contraEErange):
        contra_choose = np.array([[EE,0,0,0],[0,contraEIrange[c],0,0]])
        ...
        plotbothsidesheatmap(n_t,rEms,pulse_vals,contra_weights,offsetcontra,contra_dur,perturb_input,plotti = simname[sim],ti = simname[sim] + '_')
...
#calc in main code
# wEEadj / abs(wEIadj) for interseg axis of weights
# wEEcontra / abs(wEIcontra) for contra axis of weights
...
###STOPPED HERE
#figure out how to do the comparisons - do you want contra proportions, and if so, how do you want to store phi's according to the proportions of E/I for each weight type
#then try this and plot outcomes
#compare the phase diagram to that of the rings model
#compare qualitative and also the quant thresholds for relationships -- holds up with those I found in rings?
